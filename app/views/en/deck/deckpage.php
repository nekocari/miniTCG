	...
	<div class="row">
		...
		<?php if(!$deck->hasVoted($this->login)){ ?>
			<form method="POST" action="" class="col text-right">
				<button class="btn btn-primary" role="submit" name="vote"><i class="fas fa-thumbs-up"></i> vote</button>
			</form>
		<?php }else{ ?>
			<div class="col text-right">You have already voted!</div>
		<?php } ?>
	</div>
	<hr>
<?php } ?>
...
<div class="d-flex flex-wrap">
	...
	<div class="my-2 col-12 col-md-8">
		...
		<?php if($deck->onWishlist($this->login)){ ?>
			<form method="post" class="text-right"><button class="btn btn-outline-danger" name="wishlist" value="remove"><i class="far fa-heart"></i> <del>wishlist</del></button></form>
		<?php }else{ ?>
			<form method="post" class="text-right"><button class="btn btn-primary" name="wishlist" value="add"><i class="fas fa-heart"></i> wishlist</button></form>
		<?php } ?>
...
<div class="d-flex flex-wrap">
	<div class="col-12 col-md-6 text-left">
    	<h3 class="m-0"><i class="fas fa-folder-plus"></i> Collectors</h3>

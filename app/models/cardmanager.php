            ...
            switch($this->query_style){
                case 'exists':
                    $sql = "SELECT c.*,
                            ...
                            WHERE c.owner = ".$this->member_id." AND c.status = '$status' $where_partial
                            ORDER BY name ASC";
                    break;
                default:
                    $sql = "SELECT c.*, collect.flag as collect_flag, keep.flag as keep_flag, mastered.flag as mastered_flag, in_keep.flag as in_keep_flag, in_collect.flag as in_collect_flag
                            FROM cards c $join_partial
                            LEFT JOIN (SELECT DISTINCT deck, 1 as flag FROM cards WHERE owner = ".$this->member_id." AND status = 'collect')
                                collect ON collect.deck = c.deck
                            LEFT JOIN (SELECT DISTINCT deck, 1 as flag FROM cards WHERE owner = ".$this->member_id." AND status = 'keep')
                            ...
                            LEFT JOIN (SELECT DISTINCT deck, number, status, 1 as flag FROM cards WHERE owner = ".$this->member_id." AND status = 'collect')
                                in_collect ON in_collect.deck = c.deck AND in_collect.number = c.number
                            LEFT JOIN (SELECT DISTINCT deck, number, status, 1 as flag FROM cards WHERE owner = ".$this->member_id." AND status = 'keep')

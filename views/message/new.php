...
<form name="new-message" method="POST" action="">
    <div class="card my-4">
    	...
    	<div class="card-body">
        	<div class="row">
        		...
        		<div class="col-8">
        			<select class="form-control" name="recipient" required>
        				<option value=""></option>
        				<?php foreach($recipients as $recipient){ if($recipient->getId() != Login::getUser()->getId()) { ?>
        	...
        	<small>HTML ist nicht erlaubt!</small>
    	</div>
    </div>
    ...
    <input type="hidden" name="sender" value="<?php echo Login::getUser()->getId(); ?>">
</form>